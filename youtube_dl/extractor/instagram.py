...
class InstagramIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        # Извлекаем JSON-данные, содержащие информацию о видео
        json_data = self._search_regex(
            r'window\.__additionalDataLoaded\s*\(\s*[^,]+,\s*({.+?})\s*\);',
            webpage, 'json_data', default=None
        )

        if not json_data:
            raise ExtractorError('Failed to extract JSON data from Instagram')

        json_data = json.loads(json_data)
        media = json_data.get('graphql', {}).get('shortcode_media')

        if not media:
            raise ExtractorError('Failed to extract media data')

        # Извлекаем заголовок видео
        title = media.get('edge_media_to_caption', {}).get('edges', [{}])[0].get('node', {}).get('text', 'Instagram Video')
        ...
        # Извлекаем URL миниатюры видео
        thumbnail = media.get('display_url')
        
        # Извлекаем информацию об авторе видео
        uploader = media.get('owner', {}).get('username')
        uploader_id = media.get('owner', {}).get('id')
        
        # Извлекаем timestamp публикации
        timestamp = media.get('taken_at_timestamp')

        return {
            'id': video_id,  # Уникальный идентификатор видео
            'title': title,  # Заголовок видео
            'url': video_url,  # Прямая ссылка на видео
            'ext': 'mp4',  # Формат видео
            'thumbnail': thumbnail,  # Ссылка на миниатюру
            'uploader': uploader,  # Имя пользователя, загрузившего видео
            'uploader_id': uploader_id,  # Идентификатор пользователя
            'timestamp': timestamp,  # Время загрузки видео
        }
